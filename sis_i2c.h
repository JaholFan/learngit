...
unsigned int SIS_SLAVE_ADDR = 0x10;
#else
#define SIS_SLAVE_ADDR					0x10 //0x5c
#endif
...
#ifdef CONFIG_TOUCHSCREEN_COMPAT
unsigned int SIS_INT_PORT = (PAD_GPIO_B + 27);
unsigned int SIS_RST_PORT = (PAD_GPIO_B + 26);
#else
#define SIS_INT_PORT    (PAD_GPIO_B + 27) // (PAD_GPIO_D + 16)
#define SIS_RST_PORT    (PAD_GPIO_B + 26)
#endif
//#define SIS_INT_IRQ     PB_PIO_IRQ((PAD_GPIO_D + 16))

/* Interrupt setting and modes */
#define _I2C_INT_ENABLE					//  ON/OFF
#ifdef CONFIG_TOUCHSCREEN_COMPAT
unsigned int GPIO_IRQ = (PAD_GPIO_B + 27);
#else
#define GPIO_IRQ 					(PAD_GPIO_B + 27) // (PAD_GPIO_D + 16) //((PAD_GPIO_B + 27)) //	133
#endif

#define SIS_GPIO_AS_INT(pin)            do{\
